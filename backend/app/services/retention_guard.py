...
def forbid_disk_writes():
    ...
    def _guarded_os_open(path: Any, flags: int, mode: int = 0o777, *args: Any, **kwargs: Any):
        if _has_write_flags(flags):
            raise DiskWriteViolation(f"Disk write attempted through os.open() for: {path}")
    ...
    with ExitStack() as stack:
        ...
        stack.enter_context(patch("io.open", _guarded_io_open))
        stack.enter_context(patch("os.open", _guarded_os_open))
...
def _has_write_flags(flags: int) -> bool:
    ...
    if access_mode in (os.O_WRONLY, os.O_RDWR):
        return True
    for flag_name in ("O_CREAT", "O_TRUNC", "O_APPEND"):
